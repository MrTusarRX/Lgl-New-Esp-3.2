...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Purulia X Tusar</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">A simple ESP Project by PuruliaCheatz X MrTusarRX</p> | "
                                     "https://t.me/PuruliaCheatz | join our telegram channel for more useful modding tools and sources absolutely free</p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by PuruliaKing007 X MrTusarRX"),ToastLength::LENGTH_LONG);
